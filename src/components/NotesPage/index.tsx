    ...
    <div className="p-4 sm:p-8">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 sm:px-6 py-2 sm:py-3 bg-indigo-600 text-white rounded-lg hover:bg-indigo-800 text-sm sm:text-base transition w-full sm:w-auto justify-center sm:justify-start"
        >
      ...
      <div className="mb-6">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 sm:py-3 border rounded-lg focus:outline-none focus:ring-2  text-sm sm:text-base"
          />
        </div>
      </div>
        ...
        <div className="bg-[#131313] rounded-xl border p-4 sm:p-6 mb-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <textarea
              ...
              className="w-full px-4 py-3 border rounded-lg focus:outline-none focus:ring-2  resize-none min-h-40"
              required
            />
            ...
            <div className="flex gap-2 pt-4">
              ...
              <button
                ...
                className="flex-1 bg-indigo-600 text-white font-semibold py-2 rounded-lg hover:bg-indigo-800 disabled:opacity-50 disabled:cursor-not-allowed transition flex items-center justify-center gap-2 text-sm sm:text-base"
              >
            ...
            {/* Preview */}
            {showPreview && (
              <div className="mt-4 p-4 bg-[#131313] rounded-lg border">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
        {filteredNotes.map((note) => (
          <div
            key={note.id}
            className="bg-[#131313] rounded-xl border hover:shadow-md transition-shadow cursor-pointer"
            onClick={() => {
          ...
          >
            <div className="p-4 sm:p-6">
              <div className="flex justify-between items-start mb-4">
                ...
                <span className="text-xs sm:text-sm text-white ml-2 shrink-0">
                  {new Date(note.date).toLocaleDateString("ru-RU")}
                </span>
              </div>
              <p className="text-white font-semibold text-xl line-clamp-3 mb-4">
                {note.content}
              </p>
              <div className="flex items-center justify-between pt-4 border-t gap-2 flex-wrap">
                ...
                <div className="flex gap-1">
                  <button
                    ...
                    className="bg-indigo-600 text-white font-medium text-xs sm:text-sm hover:bg-indigo-800 px-2 py-1 rounded transition"
                  >
                    Изменить
                  </button>
                  <button
        ...
        {!showAddForm && (
          <div
            onClick={() => setShowAddForm(true)}
            className="border-2 border-dashed border-gray-300 rounded-xl flex flex-col items-center justify-center p-8 sm:p-12  hover:bg-gray-800 transition-colors cursor-pointer"
          >
            ...
            <p className="text-gray-200 text-xs sm:text-sm mt-2 text-center">
      ...
      {selectedNote && !showAddForm && (
        ...
        >
          <div
            className="bg-[#131313] rounded-t-2xl sm:rounded-2xl w-full sm:max-w-2xl max-h-[90vh] overflow-y-auto"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="sticky top-0 bg-[#131313] border-b p-4 sm:p-6 flex justify-between items-start">
              <div className="flex-1 min-w-0">
                <h2 className="text-xl sm:text-2xl font-bold text-white wrap-break-word">
                  {selectedNote.title}
                </h2>
                <p className="text-sm sm:text-base text-gray-200 mt-1">
                  ...
                  {selectedNote.subject && selectedNote.date && (
                    <span> • </span>
                  )}
                  {selectedNote.date && (
                    <span>
                      {new Date(selectedNote.date).toLocaleDateString("ru-RU")}
                    </span>
                  )}
                </p>
              </div>
              <button
                onClick={() => setSelectedNote(null)}
                className="p-2 hover:bg-gray-800 rounded-lg transition shrink-0 ml-2"
              >
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <div className="sticky bottom-0 bg-[#131313] border-t p-4 sm:p-6 flex gap-2">
              <button
                onClick={() => {
