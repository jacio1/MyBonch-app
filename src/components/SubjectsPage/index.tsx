...
import {
  Plus, Loader, BookOpen, MapPin, Trash2, X, Save, Edit2,
  Star, AlertTriangle, Settings, ChevronDown, ChevronUp, Clock,
} from 'lucide-react';
...
function TimingSettings() {
  const { timings, addTiming, updateTiming, deleteTiming, resetTimingsToDefault } = useData();
  ...
  const handleBlur = async (timing: Timing) => {
    ...
    try {
      ...
      setLocalEdits((prev) => { const next = { ...prev }; delete next[timing.id]; return next; });
    } catch {
  ...
  const handleDelete = async (id: number) => {
    try { await deleteTiming(id); } catch { alert('Ошибка удаления'); }
  };

  const handleReset = async () => {
    if (!confirm('Сбросить все тайминги к значениям по умолчанию?')) return;
    try { await resetTimingsToDefault(); } catch { alert('Ошибка сброса'); }
  };
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            <p className="font-semibold text-gray-900 dark:text-white">Время пар</p>
            <p className="text-xs text-gray-500 dark:text-gray-400">
              {timings.length} из 8 пар настроено · изменения сохраняются автоматически
            </p>
          </div>
        </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-md w-full" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-4">
          ...
          {conflictSchedule && (
            <div className="mt-4 p-3 bg-red-50 dark:bg-red-950/30 rounded-lg border border-red-200 dark:border-red-800">
              <p className="text-sm font-medium text-red-800 dark:text-red-200">Конфликтующая пара:</p>
        ...
        <div className="border-t border-gray-200 dark:border-gray-700 p-4">
          <button onClick={onClose} className="w-full bg-red-600 hover:bg-red-700 text-white py-2 rounded-lg transition">
            Понятно
          </button>
        </div>
      </div>
...
const PresetModal = ({ isOpen, onClose, onSubmit, isSubmitting, formData, onFormChange, editingId }: any) => {
  if (!isOpen) return null;
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-md w-full" onClick={(e) => e.stopPropagation()}>
        <div className="border-b border-gray-200 dark:border-gray-700 p-4 flex justify-between items-center">
          ...
          <button onClick={onClose} className="p-1 hover:bg-gray-100 dark:hover:bg-gray-700 rounded text-gray-500 dark:text-gray-400 transition">
            <X className="h-5 w-5" />
          </button>
        </div>
        <form onSubmit={onSubmit} className="p-4 space-y-4">
          ...
          <div className="flex gap-2 pt-2">
            ...
            <button type="button" onClick={onClose} className="flex-1 bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 text-gray-900 dark:text-white py-2 rounded-lg transition">
              Отмена
            </button>
          </div>
        </form>
...
const ScheduleModal = ({ isOpen, onClose, onSubmit, isSubmitting, formData, onFormChange, presetId, fixedDay, timings }: any) => {
  if (!isOpen) return null;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-md w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <div className="border-b border-gray-200 dark:border-gray-700 p-4 flex justify-between items-center">
          <div>
            <h3 className="text-lg font-bold text-gray-900 dark:text-white">Добавить пару</h3>
            <p className="text-sm text-indigo-600 dark:text-indigo-400 font-medium">{DAYS_RU[fixedDay]}</p>
          </div>
          <button onClick={onClose} className="p-1 hover:bg-gray-100 dark:hover:bg-gray-700 rounded text-gray-500 dark:text-gray-400 transition">
            <X className="h-5 w-5" />
          </button>
        ...
        <form onSubmit={(e) => onSubmit(e, presetId)} className="p-4 space-y-4">
                ...
                {timings.map((t: Timing) => {
                  ...
                  return (
                    <button
                      key={t.id}
                      type="button"
                      onClick={() => onFormChange({ ...formData, start_time: t.start_time, end_time: t.end_time })}
                    ...
                    >
                      ...
                      <p className="text-xs opacity-75 mt-0.5">{formatTime(t.start_time)} — {formatTime(t.end_time)}</p>
                    </button>
                  );
                })}
              </div>
          ...
          {formData.start_time && (
            <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-700/50 rounded-lg px-3 py-2">
              ...
              <span>Выбрано: {formData.start_time} — {formData.end_time}</span>
            </div>
          )}

          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Аудитория</label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Важная пара</label>
            <button
              type="button"
              onClick={() => onFormChange({ ...formData, is_important: !formData.is_important })}
              className={`flex items-center gap-2 px-4 py-2 rounded-lg transition ${
                formData.is_important ? 'bg-yellow-500 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300'
              }`}
            >
          ...
          <div className="flex gap-2 pt-2">
            <button
              type="submit"
              disabled={isSubmitting || timings.length === 0 || !formData.start_time}
...
function WeekGrid({ preset, onAddSchedule, onDeleteSchedule }: {
  preset: any;
    ...
    <div className="w-full">
      <div className="flex flex-col gap-2">
        {[0, 1, 2, 3, 4, 5].map((day) => {
          ...
          return (
            <div key={day} className="bg-gray-50 dark:bg-gray-900/40 rounded-xl border border-gray-200 dark:border-gray-700 p-3">
                    ...
                    <div className="flex items-start justify-between gap-1">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1 flex-wrap">
                          {s.is_important && <Star className="h-3 w-3 text-yellow-500 fill-current flex-shrink-0" />}
...
export default function PresetsPage() {
  ...
  const {
    presets, timings, loading, error,
    addPreset, updatePreset, deletePreset,
    addPresetSchedule, deletePresetSchedule,
  } = useData();
  ...
  const [scheduleTargetPresetId, setScheduleTargetPresetId] = useState<number | null>(null);
  const [scheduleTargetDay, setScheduleTargetDay] = useState(0);

  const [showErrorModal, setShowErrorModal] = useState(false);
  const [errorModalData, setErrorModalData] = useState({ title: '', message: '', conflictSchedule: null as any });
  ...
  const checkOverlap = useCallback(
    (presetId: number, day: number, start: string, end: string, excludeId?: number) => {
      const preset = presets.find((p) => p.id === presetId);
      if (!preset?.schedules) return { overlaps: false };
      for (const s of preset.schedules.filter((s) => s.day_of_week === day && s.id !== excludeId)) {
  ...
  const handleSubmitPreset = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.name) { alert('Введите название пресета'); return; }
    setIsSubmitting(true);
    try {
      editingPresetId ? await updatePreset(editingPresetId, formData) : await addPreset(formData);
      resetForm();
  ...
  const handleAddSchedule = async (e: React.FormEvent, presetId: number) => {
    e.preventDefault();
    if (!scheduleForm.subject_name) { alert('Введите название предмета'); return; }
    if (!scheduleForm.start_time) { alert('Выберите время пары'); return; }

    const { overlaps, overlappingSchedule } = checkOverlap(
      presetId, scheduleTargetDay, scheduleForm.start_time, scheduleForm.end_time
    );
    if (overlaps) {
      setErrorModalData({
        title: 'Конфликт времени!',
        message: 'Нельзя добавить пару в это время — она пересекается с существующей парой.\n\nВыберите другое время или удалите конфликт.',
    ...
    setIsSubmitting(true);
    try {
      await addPresetSchedule(presetId, { ...scheduleForm, day_of_week: scheduleTargetDay, preset_id: presetId });
      setScheduleModalOpen(false);
    } catch { alert('Ошибка при добавлении пары'); }
    finally { setIsSubmitting(false); }
  };
  ...
  return (
    <div className="p-4 sm:p-8 transition-colors">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
          <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white">Настройки расписания</h2>
          <p className="text-gray-600 dark:text-gray-400 text-sm sm:text-base">{presets.length} пресетов создано</p>
        </div>
        <button
        ...
        {filteredPresets.map((preset) => (
          <div key={preset.id} className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4 sm:p-6 hover:shadow-md transition">
            <div className="flex justify-between items-start mb-5">
              <div className="flex-1 min-w-0">
                <h3 className="font-bold text-base sm:text-lg text-gray-900 dark:text-white">{preset.name}</h3>
                {preset.description && (
                  <p className="text-sm text-gray-600 dark:text-gray-400 mt-1 line-clamp-2">{preset.description}</p>
                )}
              ...
              <div className="flex gap-1 ml-2 flex-shrink-0">
                ...
                <button
                  onClick={() => confirm('Удалить этот пресет?') && deletePreset(preset.id)}
            ...
            <WeekGrid preset={preset} onAddSchedule={openScheduleModal} onDeleteSchedule={deletePresetSchedule} />
          </div>
        ))}
      </div>
